...
# beacon.conf
	...
	# User to switch to after binding to all sockets as root. Default: beacon. If the
	...
	# beacon will ensure reasonable owner/permissions on the working, data and config
	# directories (and files), and beacon binary (if present). (optional)
	NoFixPermissions: false

	# Full hostname of system, e.g. mail.<domain>
	Hostname:

	# If enabled, a single DNS TXT lookup of _updates.xbeacon.nl is done every 24h to
	# check for a new release. Each time a new release is found, a changelog is
	# fetched from https://updates.xbeacon.nl/changelog and delivered to the postmaster
	# mailbox. (optional)
	CheckUpdates: false
	...
	Listeners:
		x:
			...
			# it is better to explicitly specify the IPs you want to use for email, as beacon
			...
			TLS:
				...
				# privileged root process and passed to the unprivileged beacon process, so no
			...
			# up a tunnel (e.g. WireGuard) and add its IP to the beacon 'internal' listener.
			# (optional)
			AdminHTTP:
				Enabled: false
			...
			# Serve /debug/pprof/ for profiling a running beacon instance. Do not enable this on
	...
	Domains:
		x:
			...
			MTASTS:
				...
				# change a policy, first change it in beacon, then update the DNS record.
				PolicyID:
	...
	WebHandlers:
		-
			...
			WebStatic:
				...
				# are relative to the working directory of beacon.
				Root:
...
# Example transport
	...
	# Snippet for beacon.conf, defining a transport called Example that connects on the
