...
func TestWebserver(t *testing.T) {
	...
	beacon.ConfigStaticPath = filepath.FromSlash("../testdata/webserver/beacon.conf")
	beacon.ConfigDynamicPath = filepath.Join(filepath.Dir(beacon.ConfigStaticPath), "domains.conf")
	beacon.MustLoadConfig(true, false)

	loadStaticGzipCache(beacon.DataDirPath("tmp/httpstaticcompresscache"), 1024*1024)
	...
	test("GET", "http://redir.beacon.example", nil, http.StatusPermanentRedirect, "", map[string]string{"Location": "https://beacon.example/"})
	...
	test("GET", "http://beacon.example/static/", accgzip, http.StatusOK, "", map[string]string{"X-Test": "beacon", "Content-Encoding": "gzip"})       // index.html
	test("GET", "http://beacon.example/static/dir/hi.txt", accgzip, http.StatusOK, "", map[string]string{"X-Test": "beacon", "Content-Encoding": ""}) // too small to compress
	test("GET", "http://beacon.example/static/dir/", accgzip, http.StatusOK, "", map[string]string{"X-Test": "beacon", "Content-Encoding": "gzip"})   // listing
	test("GET", "http://beacon.example/static/dir", accgzip, http.StatusTemporaryRedirect, "", map[string]string{"Location": "/static/dir/"})      // redirect to dir
	test("GET", "http://beacon.example/static/bogus", accgzip, http.StatusNotFound, "", map[string]string{"Content-Encoding": ""})

	test("GET", "http://beacon.example/nolist/", nil, http.StatusOK, "", nil)            // index.html
	test("GET", "http://beacon.example/nolist/dir/", nil, http.StatusForbidden, "", nil) // no listing

	test("GET", "http://beacon.example/tls/", nil, http.StatusPermanentRedirect, "", map[string]string{"Location": "https://beacon.example/tls/"}) // redirect to tls

	test("GET", "http://beacon.example/baseurl/x?y=2", nil, http.StatusPermanentRedirect, "", map[string]string{"Location": "https://tls.beacon.example/baseurl/x?q=1&y=2#fragment"})
	test("GET", "http://beacon.example/pathonly/old/x?q=2", nil, http.StatusTemporaryRedirect, "", map[string]string{"Location": "http://beacon.example/pathonly/new/x?q=2"})
	test("GET", "http://beacon.example/baseurlpath/old/x?y=2", nil, http.StatusPermanentRedirect, "", map[string]string{"Location": "//other.beacon.example/baseurlpath/new/x?q=1&y=2#fragment"})

	test("GET", "http://beacon.example/strip/x", nil, http.StatusBadGateway, "", nil)   // no server yet
	test("GET", "http://beacon.example/nostrip/x", nil, http.StatusBadGateway, "", nil) // no server yet
	...
	beacon.Conf.Dynamic.WebHandlers[len(beacon.Conf.Dynamic.WebHandlers)-2].WebForward.TargetURL = serverURL
	beacon.Conf.Dynamic.WebHandlers[len(beacon.Conf.Dynamic.WebHandlers)-1].WebForward.TargetURL = serverURL

	test("GET", "http://beacon.example/strip/x", badForwarded, http.StatusOK, "/a/x", map[string]string{
		"X-Test":            "beacon",
		"X-Forwarded-For":   "192.0.2.1", // IP is hardcoded in Go's src/net/http/httptest/httptest.go
		"X-Forwarded-Proto": "http",
		"X-Forwarded-Host":  "beacon.example",
		"X-Forwarded-Ext":   "",
	})
	test("GET", "http://beacon.example/nostrip/x", map[string]string{"X-OK": "ok"}, http.StatusOK, "/a/nostrip/x", map[string]string{"X-Test": "beacon"})

	test("GET", "http://beacon.example/bogus", nil, http.StatusNotFound, "", nil)         // path not registered.
	test("GET", "http://bogus.beacon.example/static/", nil, http.StatusNotFound, "", nil) // domain not registered.
	...
	loadStaticGzipCache(beacon.DataDirPath("tmp/httpstaticcompresscache"), 0)
	npaths = len(staticgzcache.paths)
	if npaths != 0 {
...
func TestWebsocket(t *testing.T) {
	...
	beacon.ConfigStaticPath = filepath.FromSlash("../testdata/websocket/beacon.conf")
	beacon.ConfigDynamicPath = filepath.Join(filepath.Dir(beacon.ConfigStaticPath), "domains.conf")
	beacon.MustLoadConfig(true, false)
	...
	// warning: it is not normally allowed to access the dynamic config without lock. don't propagate accesses like this!
	beacon.Conf.Dynamic.WebHandlers[len(beacon.Conf.Dynamic.WebHandlers)-1].WebForward.TargetURL = backendURL
