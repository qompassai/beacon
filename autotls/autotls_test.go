...
func TestAutotls(t *testing.T) {
	...
	m, err := Load("test", "../testdata/autotls", "beacon@localhost", "https://localhost/", "", nil, getPrivateKey, shutdown)
	if err != nil {
	...
	if err := m.HostPolicy(context.Background(), "beacon.example"); err == nil || !errors.Is(err, errHostNotAllowed) {
		t.Fatalf("hostpolicy, got err %v, expected errHostNotAllowed", err)
	}
	m.SetAllowedHostnames(log, dns.MockResolver{}, map[dns.Domain]struct{}{{ASCII: "beacon.example"}: {}}, nil, false)
	l = m.Hostnames()
	if !reflect.DeepEqual(l, []dns.Domain{{ASCII: "beacon.example"}}) {
		t.Fatalf("hostnames, got %v, expected single beacon.example", l)
	}
	if err := m.HostPolicy(context.Background(), "beacon.example"); err != nil {
		t.Fatalf("hostpolicy, got err %v, expected no error", err)
	}
	if err := m.HostPolicy(context.Background(), "beacon.example:80"); err != nil {
		t.Fatalf("hostpolicy, got err %v, expected no error", err)
	}
	if err := m.HostPolicy(context.Background(), "other.beacon.example"); err == nil || !errors.Is(err, errHostNotAllowed) {
		t.Fatalf("hostpolicy, got err %v, expected errHostNotAllowed", err)
	}

	ctx := context.Background()
	cache := m.Manager.Cache
	if _, err := cache.Get(ctx, "beacon.example"); err == nil || !errors.Is(err, autocert.ErrCacheMiss) {
		t.Fatalf("cache get for absent entry: got err %v, expected autocert.ErrCacheMiss", err)
	}
	if err := cache.Put(ctx, "beacon.example", []byte("test")); err != nil {
		t.Fatalf("cache put for absent entry: got err %v, expected error", err)
	}
	if data, err := cache.Get(ctx, "beacon.example"); err != nil || string(data) != "test" {
		t.Fatalf("cache get: got err %v data %q, expected nil, 'test'", err, data)
	}
	if err := cache.Put(ctx, "beacon.example", []byte("test2")); err != nil {
		t.Fatalf("cache put for absent entry: got err %v, expected error", err)
	}
	if data, err := cache.Get(ctx, "beacon.example"); err != nil || string(data) != "test2" {
		t.Fatalf("cache get: got err %v data %q, expected nil, 'test2'", err, data)
	}
	if err := cache.Delete(ctx, "beacon.example"); err != nil {
	...
	m2, err := Load("test2", "../testdata/autotls", "beacon@localhost", "https://localhost/", "", nil, nil, shutdown)
	if err != nil {
