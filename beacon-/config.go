...
var pkglog = mlog.New("beacon", nil)
...
func writeDynamic(ctx context.Context, log mlog.Log, c config.Dynamic) error {
	...
	if err := beaconio.SyncDir(log, filepath.Dir(ConfigDynamicPath)); err != nil {
...
func ParseConfig(ctx context.Context, log mlog.Log, p string, checkOnly, doLoadTLSKeyCerts, checkACMEHosts bool) (c *Config, errs []error) {
	...
	f, err := os.Open(p)
	if err != nil {
		if os.IsNotExist(err) && os.Getenv("BEACONCONF") == "" {
			return nil, []error{fmt.Errorf("open config file: %v (hint: use beacon -config ... or set BEACONCONF=...)", err)}
		}
...
// for starting beacon. If checkOnly is set no substantial changes are made, like
...
func PrepareStaticConfig(ctx context.Context, log mlog.Log, configFile string, conf *Config, checkOnly, doLoadTLSKeyCerts bool) (errs []error) {
	...
	if err != nil {
		...
		if err != nil {
			addErrorf("parsing unknown user %s as uid: %v (hint: add user beacon with \"useradd -d $PWD beacon\" or specify a different username on the quickstart command-line)", c.User, err)
		} else {
	...
	// - run 1: only look at public listener (and host matching beacon host name)
	// - run 2: all listeners (and host matching beacon host name)
