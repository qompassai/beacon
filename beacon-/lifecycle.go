...
// RestorePassedFiles reads addresses from $BEACON_SOCKETS and paths from $BEACON_FILES
...
func RestorePassedFiles() {
	s := os.Getenv("BEACON_SOCKETS")
	if s == "" {
		var linuxhint string
		if runtime.GOOS == "linux" {
			linuxhint = " If you updated from v0.0.1, update the beacon.service file to start as root (privileges are dropped): ./beacon config printservice >beacon.service && sudo systemctl daemon-reload && sudo systemctl restart beacon."
		}
		pkglog.Fatal("beacon must be started as root, and will drop privileges after binding required sockets (missing environment variable BEACON_SOCKETS)." + linuxhint)
	}
...
func Listen(network, addr string) (net.Listener, error) {
	...
	// On windows, we cannot duplicate a socket. We don't need to for beacon localserve
	// with FilesImmediate.
	if !FilesImmediate {
...
// an immediate read/write deadline shortly after initiating beacon shutdown, after
...
// some time after beacon shutdown is initiated.
