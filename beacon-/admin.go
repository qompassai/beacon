...
func dkimKeyNote(kind string, selector, domain dns.Domain) string {
	...
	s += fmt.Sprintf(", generated by beacon on %s", time.Now().Format(time.RFC3339))
	return s
}
...
func DomainRecords(domConf config.Domain, domain dns.Domain, hasDNSSEC bool, certIssuerDomainName, acmeAccountURI string) ([]string, error) {
	...
	// The first line with ";" is used by ../testdata/integration/beaconacmepebble.sh and
	// ../testdata/integration/beaconmail2.sh for selecting DNS records
	records := []string{
	...
	if sts := domConf.MTASTS; sts != nil {
		...
		records = append(records,
			...
			"; domain or because beacon.conf does not have a listener with MTA-STS configured.",
			"",
		)
	}
	...
	if certIssuerDomainName != "" {
		...
		if acmeAccountURI != "" {
			...
			records = append(records,
				";",
				"; Optionally limit certificates for this domain to the account ID and methods used by beacon.",
			...
			// ../testdata/integration/beaconacmepebble.sh and ../testdata/integration/beaconmail2.sh
